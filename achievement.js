...
function checkFirstWinAchievement(levelKey) {
  // Fetch highscores for the current level
  const highscores = JSON.parse(localStorage.getItem(levelKey)) || [];

  // If there are entries in the highscores, unlock "First Win"
  if (highscores.length > 0) {
      unlockAchievement("firstWin-tracker", "First Win");
  }
}
